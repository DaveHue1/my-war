...
execute if score #mywar.replenish mywar.dummy matches 5.. run execute as @e[type=marker,tag=mywar.village,tag=mywar.village_done,tag=!mywar.village_captured,limit=1] at @s run function mywar:entity/village/replenish/replenish 
execute if score #mywar.replenish mywar.dummy matches 5.. run scoreboard players set #mywar.replenish mywar.dummy 0
